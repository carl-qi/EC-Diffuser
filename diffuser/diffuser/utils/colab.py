...
def run_diffusion(model, dataset, obs, goal=None, n_samples=1, device='cuda:0', **diffusion_kwargs):
  ...
  if goal is not None:
     ...
     conditions[dataset.horizon - 1] = to_torch(goal, device=device)

  diffusion = model.conditional_sample(conditions,
        return_chain=True, verbose=False, **diffusion_kwargs)
  ## [ n_samples x (n_diffusion_steps + 1) x horizon x (action_dim + observation_dim)]
  diffusion = diffusion.chains.detach().cpu().numpy()
...
def show_sample(renderer, observations, filename='sample.mp4', savebase='./'):
    ...
    for rollout in observations:
        ...
        img = renderer._renders_og(rollout, (256,256))
        images.append(img)
    breakpoint()
