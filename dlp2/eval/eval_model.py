...
from dlp2.datasets.libero_ds import LiberoSpatial
from dlp2.datasets.kitchen_ds import KitchenDS
from dlp2.datasets.langtable_ds import LanguageTableDatasetImage
# util functions
...
def evaluate_validation_elbo(model, ds, data_root_dir, epoch, batch_size=100, recon_loss_type="vgg", device=torch.device('cpu'),
    ...
    elif ds == "franka_kitchen":
        image_size = 128
        dataset = KitchenDS(data_root_dir, mode='valid', res=image_size)
    elif ds == "libero_spatial":
        image_size = 128
        dataset = LiberoSpatial(data_root_dir, mode='valid', res=image_size)
    elif ds == "langtable":
        image_size = 128
        dataset = LanguageTableDatasetImage(data_root_dir, mode='valid', res=image_size)
    else:
    ...
    for batch in dataloader:
        if ds == 'panda_push' or ds == 'libero_spatial' or ds == "franka_kitchen":
