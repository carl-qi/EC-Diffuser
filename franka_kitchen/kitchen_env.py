...
import logging
import d4rl.kitchen.adept_envs as adept_envs # type: ignore
import einops
import gym
import numpy as np
import torch
from d4rl.kitchen.adept_envs.franka.kitchen_multitask_v0 import KitchenTaskRelaxV1 # type: ignore
...
class KitchenBase(KitchenTaskRelaxV1):
    ...
    def set_task_goal(self, one_hot_indices):
        ...
        print("Setting task goal to {}".format(self.tasks_to_complete))
        self.TASK_ELEMENTS = self.tasks_to_complete
        self.goal = self._get_task_goal(task=self.tasks_to_complete, actually_return_goal=True)
        return self.goal
...
class KitchenAllV0(KitchenBase):
    ...
    def reset_model_with_state(self, reset_pos):
...
class KitchenWrapper(gym.Wrapper):
    ...
    def reset_model_with_state(self, reset_pos, process_img=True):
        obs = self.env.reset_model_with_state(reset_pos)
        if self.visual_input:
            return_obs = self.render(mode="rgb_array")
            if process_img:
                return self.preprocess_img(return_obs)
...
register(
    id="kitchen-v0",
    entry_point="franka_kitchen.kitchen_env:KitchenAllV0",
