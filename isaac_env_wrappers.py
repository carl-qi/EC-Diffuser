...
from dlp_utils import load_pretrained_rep_model, load_latent_classifier, get_dlp_rep, extract_dlp_image, check_config
...
class IsaacPandaPushGoalSB3Wrapper(GoalEnv, SB3VecEnvAdapter):
    def __init__(self, env, obs_mode, n_views, latent_rep_model, latent_classifier, reward_cfg, smorl=False, collect_images=True, **kwargs):
        ...
        if self.obs_mode == 'state':
            ...
            goal = self._get_state_obs(obs_dict)
            ...
            d_goal_shape = [1, goal_shape[-1]] if self.smorl else [self.num_goal_objects+1, goal_shape[-1]] if self.push_t else goal_shape
        ...
        elif self.obs_mode in ['dlp', 'vae', 'vqvae', 'slot']:
        ...
        if self.collect_images:
            self.observation_space = spaces.Dict({
                "observation": spaces.Box(low=obs_low, high=obs_high, shape=obs_shape, dtype=obs_dtype),
    ...
    def reset(self, set_init_states=None, set_goal_states=None):
        """
        ...
        if set_goal_states is not None:
            goal_obs_dict = self.get_set_goal(set_goal_states)
        else:
            goal_obs_dict = self.get_random_goal()  # resets env

        if set_init_states is not None:
            obs_dict = self.env.reset(set_init_states=set_init_states)
        else:
            obs_dict = self.env.reset()
        ...
        if self.obs_mode == 'state':
            ...
            achieved_goal = self._get_state_obs(obs_dict)
            self.goal = self._get_state_obs(goal_obs_dict)
            if self.push_t:
                self.goal = self.goal[:, :self.num_goal_objects+1]
            if self.smorl:
        ...
        if self.push_t:
            self.goal_pos = self.goal_pos[:, :self.num_goal_objects]
    ...
    def compute_reward(self, achieved_goal, desired_goal, info={}):
        ...
        if self.push_t:
            ...
            if self.num_goal_objects < self.num_objects:
                # assumes num_objects is divisible by num_goal_objects and that colors are assigned cyclically
                d_goal = np.tile(d_goal, (1, int(np.ceil(self.num_objects / self.num_goal_objects)), 1))[:, :self.num_objects]
    ...
    def get_random_goal(self):
        ...
        # reset
        self.env.reset()
        self.goal_obs_dict = self.env.get_init_obj_state_dict()
        # post reset
    ...
    def get_set_goal(self, goal_states_dict):
        # pre reset
        self.env.goal_reset = True
        # reset
        self.env.reset(set_init_states=goal_states_dict)
        self.goal_obs_dict = self.env.get_init_obj_state_dict()
        # post reset
        self.env.goal_reset = False
        # move arm one step back
    ...
    def _image_to_latent_rep(self, image_obs, obs_mode=None):
        ...
        elif obs_mode == 'vae' or obs_mode == 'vqvae':
    ...
    def num_colors(self) -> int:
        """Get the number of object colors in the environment."""
        return self.env.num_colors

    @property
    def num_goal_objects(self) -> int:
        """Get the number of goal objects in the environment."""
        return self.env.num_goal_objects
...
if __name__ == '__main__':
    ...
    # load config files
    config = yaml.safe_load(Path('env_config/n_cubes/Config.yaml').read_text())
    isaac_env_cfg = yaml.safe_load(Path('env_config/n_cubes/IsaacPandaPushConfig.yaml').read_text())
