...
"""
Modified by Dan Haramati from https://github.com/NVlabs/oscar
"""

from isaacgym import gymapi
import torch
import numpy as np
from panda_controller.torch_utils import quat_mul, quat2mat, orientation_error, axisangle2quat, tensor_clamp
from panda_controller.base_controller import Controller
...
class OSCController(Controller):
    ...
    def update_goal(self, control_dict, command, env_ids=None, set_ori_goal=False, train=False, target_pos=False):
        """
        ...
        if train:
            ...
            if self.pos_limits is not None:
                self.goal_pos = tensor_clamp(self.goal_pos, self.pos_limits[0], self.pos_limits[1])
            ...
            if self.pos_limits is not None:
                self.goal_pos[env_ids] = tensor_clamp(self.goal_pos[env_ids], self.pos_limits[0], self.pos_limits[1])
            if set_ori_goal:
    ...
    def _compute_osc_torques(
        ...
        mm = control_dict["mm"][:, :control_dim,
             :control_dim]  # Keep mm on cpu because running the mm inverse is OOM quicker, see https://github.com/pytorch/pytorch/issues/42265
    ...
    def reset(self, control_dict, env_ids=None):
        ...
        self.update_goal(
            control_dict=control_dict,
            command=torch.zeros(n_cmds, 6, device=self.device),
            env_ids=env_ids,
            set_ori_goal=True,
        )
